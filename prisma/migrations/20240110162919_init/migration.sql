...
CREATE TABLE `LabProcedure` (
    ...
    `labProcedureCategoryId` VARCHAR(191) NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
...
CREATE TABLE `LabProcedureCategory` (
    ...
    UNIQUE INDEX `LabProcedureCategory_categoryName_key`(`categoryName`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `LabProcedure` ADD CONSTRAINT `LabProcedure_labProcedureCategoryId_fkey` FOREIGN KEY (`labProcedureCategoryId`) REFERENCES `LabProcedureCategory`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
